...
| `EXPOSE 8005` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13335"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15435"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11435"| SQLSERVER
```
...
docker network create proyecto_paradigmas4_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas4_default --restart unless-stopped `
...
# 4. mariadb
docker run -d --name mariadb --network proyecto_paradigmas4_default --restart unless-stopped `
  ...
  -p 13335:3306 mariadb:11

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver
docker run -d --name sqlserver --network proyecto_paradigmas4_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11435:1433 mcr.microsoft.com/mssql/server:2022-latest

# 7. ESPERAR a que responda de verdad… mirándolo a ojo

# 8. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_paradigmas4_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
...
docker run -d --name api-facturas --network proyecto_paradigmas4_default --restart unless-stopped `
  ...
  -p 8005:8005 api-facturas uvicorn main:app --host 0.0.0.0 --port 8005 --reload
...
```yaml
URL_API: http://api-facturas:8005
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8005` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8005` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8005:8005`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8005`, el front
...
```powershell
docker network inspect proyecto_paradigmas4_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas4` produce
la red `proyecto_paradigmas4_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas4-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas4_default has active endpoints (name:"proyecto_paradigmas4-api-facturas-1" …)
```
